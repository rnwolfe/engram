...
- ~~**Source code ingestion** via tree-sitter — file/module/symbol entities and structural edges.~~ **Removed ([ADR-010](DECISIONS.md#adr-010----remove-tree-sitter-source-ingestion-the-moat-is-off-tree-history-not-code-structure), 2026-06-29):** general AST/symbol entities mirrored current code structure (redundant with agentic file search) and taxed retrieval precision. The cross-file Kubernetes-operator semantics (kubebuilder RBAC markers, controller-runtime watches) — which a single-file read can't reveal — are preserved as a standalone, tree-sitter-free `engram ingest k8s`.
