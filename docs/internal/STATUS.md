...
> Latest release: **v0.3.2** (2026-04-24) — see [CHANGELOG.md](../../CHANGELOG.md) for the full v0.3 surface (narrative CLI, `sync`, repo-lifecycle health, visualize-in-release-binaries fix).
...
- [x] Web UI assets embedded into the compiled binary via Bun `with { type: "file" }` imports; `release.yml` runs the workspace build before `bun build --compile` so release binaries can finally serve visualize (v0.3.2, #276)
- [x] Force-tracked `packages/engram-web/dist/ui/*` removed from version control — was drifting from source; assets now generated by `bun run build` and embedded at build time (v0.3.2, #276)

#### CLI / UX

- [x] Systematic UX audit fixes for human and agent ergonomics (#251)
- [x] `/cleanup` skill for stale branch and worktree triage (#252)
- [x] `/docs-audit` skill for auditing README/STATUS drift against shipped code (v0.3.2, #276)

#### Documentation

- [x] README update for expanded source-ingest language coverage (#250)
- [x] STATUS and VISION sync with v0.2 shipped work (#253)
- [x] README + STATUS resync with v0.3 CLI surface (`sync`, `diff`, `why`, `brief`, `onboard`, `whats-new`, `update`, `plugin info`, `context --as-of`); Google Workspace promoted into the Plugin (bundled) tier (v0.3.2, #276)
...
- **ADRs**: 5 with full bodies in `docs/internal/DECISIONS.md` (001, 002, 006, 007, 008). Numbers 003–005 are referenced from STATUS/CLAUDE but lack ADR bodies — a docs debt flagged in 2026-04-24 sync; backfill or renumber as a follow-up.
